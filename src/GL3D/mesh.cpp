...
namespace GL3D {
	Mesh::Mesh(std::span<float> vertices, std::span<int> num_floats_per_attr, std::span<unsigned int> indices)
	{
	...
	void Mesh::set_vertex_attributes(unsigned int vao, unsigned int vbo, std::span<int> num_floats_per_attr) {
		glBindVertexArray(vao);
		glBindBuffer(GL_ARRAY_BUFFER, vbo);
